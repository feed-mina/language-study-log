...
type Card = { id: string; prompt: string; answer: string; explanation: string; language: string; due: string; priorityGroup: 'weak' | 'stage'; priorityLabel: string; priorityScore: number; learningStage: number; reviewedAt?: string | null; rating?: string | null };
...
export default function StudySchedule(props: Props) {
  ...
  return <div className="study-schedule">
    ...
    <div className="schedule-columns"><div>{loading ? <p className="empty-state">선택한 하루를 불러오는 중…</p> : data && <DayAgenda date={date} items={data.agenda} budget={data.dailyMinutes} canEdit={canEdit} busy={busy} onAdd={props.onAdd} onComplete={props.onComplete} onUndo={props.onUndo} onOpen={props.onOpen} onReview={item => void openReview(item)} onEdit={item => { setEditing(item); setEditDate(item.date); setTime(formatStart(item.start)); }} onCancel={item => { if (window.confirm('복습 배정을 취소할까요? 이미 저장한 카드 평가는 유지됩니다.')) void save({ action: 'session-cancel', sessionId: item.id }); }} />}
      ...
      <section className="review-planner"><p className="mini-label">SPACED REVIEW</p><h2>복습 시간 배정</h2><p>현재 도래 · 미배정 {data?.availableCount ?? 0}장</p><p className="schedule-help">약한 단어를 먼저, 같은 조건에서는 낮은 학습 단계부터 보여줍니다. 최대 50장씩 표시하고 한 번에 20장까지 배정할 수 있어요.</p>
        <form onSubmit={e => { e.preventDefault(); void save({ action: 'session-create', date, time: reviewTime, minutes: reviewMinutes, cardIds }).then(ok => { if (ok) setCardIds([]); }); }}><div className="review-card-picker">{!loading && data?.availableCards.map(card => <label key={card.id}><input type="checkbox" checked={cardIds.includes(card.id)} disabled={busy || !canEdit || (!cardIds.includes(card.id) && cardIds.length >= 20)} onChange={e => setCardIds(old => e.target.checked ? [...old, card.id] : old.filter(id => id !== card.id))} /><span><small>{card.language} · <b className={`review-priority ${card.priorityGroup}`}>{card.priorityLabel}</b></small>{card.prompt}</span></label>)}{!loading && !data?.availableCards.length && <p>지금 배정할 복습 카드가 없습니다.</p>}</div><p>선택 {cardIds.length}장 · 배정일 {date}</p><label>예상 분량 (분)<input type="number" min="1" max="600" value={reviewMinutes} onChange={e => setReviewMinutes(Number(e.target.value))} required disabled={busy} /></label><label>시작 시각 (선택)<input type="time" value={reviewTime} onChange={e => setReviewTime(e.target.value)} disabled={busy} /></label><button disabled={!canEdit || busy || loading || !cardIds.length || date < today}>선택일에 복습 배정</button></form><p className="schedule-help">오답 다시 풀기와 복습 메모는 아래 기존 학습 영역에서 이어볼 수 있습니다.</p>
      </section></div></div>
