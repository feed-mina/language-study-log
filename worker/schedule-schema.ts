// Additive schema: existing plans and learning history are never rewritten.
export const scheduleSchema = [
  ...
  `CREATE TRIGGER IF NOT EXISTS validate_review_assignment BEFORE INSERT ON review_session_cards BEGIN
    SELECT CASE WHEN NOT EXISTS (SELECT 1 FROM study_cards c JOIN review_sessions s ON s.id=NEW.session_id WHERE c.id=NEW.card_id AND c.due<=s.created_at AND s.status='planned') THEN RAISE(ABORT, 'REVIEW_CONFLICT') END;
  END`,
  ...
  `CREATE TRIGGER IF NOT EXISTS validate_schedule_mutation BEFORE INSERT ON schedule_receipts BEGIN
    SELECT CASE WHEN json_extract(NEW.fingerprint,'$.action')='plan' AND NOT EXISTS(SELECT 1 FROM study_plans WHERE id=json_extract(NEW.fingerprint,'$.planId') AND status=json_extract(NEW.fingerprint,'$.expectedStatus') AND updated_at=json_extract(NEW.fingerprint,'$.expectedUpdatedAt')) THEN RAISE(ABORT, 'REVIEW_CONFLICT') END;
    SELECT CASE WHEN json_extract(NEW.fingerprint,'$.action')='time' AND NOT EXISTS(SELECT 1 FROM study_plans WHERE id=json_extract(NEW.fingerprint,'$.planId') AND status='planned') THEN RAISE(ABORT, 'REVIEW_CONFLICT') END;
    SELECT CASE WHEN json_extract(NEW.fingerprint,'$.action') IN ('session-move','session-cancel') AND NOT EXISTS(SELECT 1 FROM review_sessions WHERE id=json_extract(NEW.fingerprint,'$.sessionId') AND status='planned') THEN RAISE(ABORT, 'REVIEW_CONFLICT') END;
  END`,
